...
def getContours(img):           #this function is used to draw contours in their appropriate places
    ...
    for cnt in contours:                #for loop is for iterating over all the contours that needs to be drawn
        ...
        if area > 600:                   #this is for filtering the noise ---> that means if the area is >800 then it means we are looking at something substantial and we need to draw contour on that and broadcast the TF for that
            ...
            if midX != 0 and midY != 0 and midX <= 640 and midY <= 480:     #as the frame of the output image is 640 x 480 pixels so this is to prevent accessing values that are out of bound by any case
                ...
                if (depth <= 0.75) and (flag_inp is True):      #over here the depth value that allows us to pick the tomatoes on the side where the bot is standing is 0.70
...
def Pcontroller():     #function for the proportional controller --> this has been used so that the bot is able to correct its path if it gets deflected form its path
    ...
    angular_vel = kp * (regions['bleft']-0.60) #when the left value of the distance as calculated by the laser is 0.60 then the angular_vel is 0 and any deflection from this value will set a certain angular_vel to correct that deflection
