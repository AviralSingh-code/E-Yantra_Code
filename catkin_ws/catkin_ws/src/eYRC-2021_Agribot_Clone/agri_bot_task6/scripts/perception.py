...
# tomatoColor = [[0, 37, 117, 0, 250, 255], [123, 106, 0, 179, 255, 255]] 
###################################################
...
def callback_depth(data_depth):             #this callback is for accessing the depth image
    ...
    try:
        bridgeDepth = CvBridge()                #for creating the bridge object
        np_arr = np.frombuffer(data_depth.data, np.uint8)
...
def main(args):                                                                         #this is the main method
    ...
    depth_sub = rospy.Subscriber("/camera/aligned_depth_to_color/image_raw", Image, callback_depth, queue_size=100)     #for setting the depth image subscriber
    image_sub = rospy.Subscriber("/camera/color/image_raw/compressed",CompressedImage, callback, queue_size=100)           #for setting the color image subscriber
    try:
