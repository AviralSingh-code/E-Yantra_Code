...
def callback_Aruco(data):
    ...
    global ArucoId          #this variable is to hold the aruco id of the marker visible
    ...
    if motionStage[0] is True:
        ...
        if pose[2] <= 0:
            ...
            if pose[1] <=8.27:      #pose[1] ---> is the y value of the bot
                ...
                linear_vel = 0.0
                angular_vel = 1.7           #while turning the angular velocity of the bot is 1.7
...
def main(args):                                                                         #this is the main method
    ...
    while not rospy.is_shutdown():                      #while loop
        ...
        ##########################################################
            ...
            '''In this section we will move the bot till the time the TF of the tomato is not available ----> the TF of the bot will only be availabe when the tomato is within a fixed depth range'''
            try:                                                                            #try to handle exceptions
                trans = tfBuffer.lookup_transform('base_link', 'obj0', rospy.Time(0))      #for getting the TF of obj0 ----> tomato in the scene
            ...
            trans = tfBuffer.lookup_transform('base_link', 'obj0', rospy.Time(0))  #the TF of the tomato is read again when the bot is completely stopped so that the tomato 
            ...
            if flagExecution is False:              #this variable is used to ensure that the steps after this are only actuated when this step is successfully actuated
                continue
            rospy.sleep(0.1)
